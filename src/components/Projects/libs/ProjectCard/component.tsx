    ...
    <div className={clsx(styles.container, className)} style={{ zIndex: 1 }}>
      <div className="px-6 py-6 md:p-0">
        <div className={clsx(rightShift && "md:text-right")}>
          <a
            href={externalLink}
            className="text-xl font-bold md:text-2xl dark:text-light-900 text-dark-900"
            target="_blank"
            rel="noreferrer"
          >
            {title}
          </a>
          <p className="text-base font-medium md:text-xl dark:text-light-700 text-dark-700">
            {subtitle}
          </p>
        </div>
        <p
        ...
        >
          {technologies.map((tech, i) => (
            <p
              className={clsx(
                "text-base dark:text-light-700 text-dark-700",
