...
export const Header: FC = () => {
    ...
    (newLanguage: Languages) => {
      return () => {
        ...
        if (newLanguage !== language) {
          const urlEncoded = new URLSearchParams({ locale: newLanguage });
          router("/", { state: urlEncoded });
          // router("/", { state: { locale: newLanguage } });
        }
      };
    },
  ...
  return (
    ...
    >
      <Container className="flex items-center justify-between w-auto py-5 md:py-9 text-dark-900 dark:text-light-900">
        <Link to="/">
          <Logo className="w-24 fill-current md:w-28 dark:text-light-900 text-dark-900" />
        </Link>
        <div className="flex items-center">
          ...
          <div className="relative ml-2 md:ml-4" ref={ref}>
            ...
            {langPicker && (
              <div className="absolute w-full p-1 mt-4 bg-pink dark:bg-light-900 rounded-md text-dark-900">
                {languages.map((currentLanguage, i) => (
                  <>
                    <button
                      className="block w-full px-2 py-1 text-left hover:bg-light-700 rounded-md transition-colors focus:outline-none"
                    ...
                    {i !== languages.length - 1 && (
                      <div className="my-1 bg-light-700 h-0.5" />
                    )}
                  </>
                ))}
              </div>
